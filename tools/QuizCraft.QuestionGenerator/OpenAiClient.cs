...
public class OpenAiClient : IDisposable
{
    ...
    private static string MontarPromptSistema(string materia, Lote lote)
    {
        ...
        return $"""
            ...
            DISTRIBUIÇÃO DE DIFICULDADE (total 25):
            - 5 questões com Difficulty=1 (muito fácil)
            - 5 questões com Difficulty=2 (fácil)
            - 10 questões com Difficulty=3 (média)
    ...
    private static object MontarCorpoRequisicao(string systemPrompt)
    {
        return new
        {
            ...
            input = new object[]
            {
                ...
                new { role = "user", content = "Gere as 25 questões agora." }
            },
            text = new
            {
                format = new
